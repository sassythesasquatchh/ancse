#ifndef LSHAPE_MESH_ADAPTIVE
#define LSHAPE_MESH_ADAPTIVE
...
BoundaryMesh createLshapeMeshAdaptive(int k, double alpha) {
  ...
  rotmat << std::cos(M_PI/4), -std::sin(M_PI/4), std::sin(M_PI/4), std::cos(M_PI/4); 
    ...
    for (int i = 0 ; i < eltsPerBigSide ; ++i ) {
        ...
        // Right edge
        vertices.row(eltsPerBigSide + i) = SE + ((double)i)/ (double)eltsPerBigSide  * (NE - SE);
        // Top edge
        if (i < eltsPerSmallSide) {
          vertices.row(2*eltsPerBigSide + i) = NE+ ((double)i)/(double)eltsPerSmallSide * (T-NE);
        }
        else {
          //vertices.row(2*eltsPerBigSide + i) = T+ std::pow(((double)(i-eltsPerSmallSide))/(double)eltsPerSmallSide,alpha) * (C-T);
          vertices.row(2*eltsPerBigSide + i) = C+ std::pow(((double)(eltsPerBigSide-i))/(double)eltsPerSmallSide,alpha) * (T-C);
        }

        if (i < eltsPerSmallSide) {
          vertices.row(3*eltsPerBigSide + i) = C+ std::pow(((double)i)/(double)eltsPerSmallSide,alpha) * (L-C);
        }
        else {
          vertices.row(3*eltsPerBigSide + i) = L+ ((double)(i-eltsPerSmallSide))/(double)eltsPerSmallSide * (SW-L);
        }
    }
