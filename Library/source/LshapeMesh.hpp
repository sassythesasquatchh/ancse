  ...
  rotmat << std::cos(M_PI/4), -std::sin(M_PI/4), std::sin(M_PI/4), std::cos(M_PI/4); 
    ...
    for (int i = 0 ; i < eltsPerBigSide ; ++i ) {
        ...
        if (i < eltsPerSmallSide) {
          ...
          vertices.row(2*eltsPerBigSide + i) = T+ ((double)(i-eltsPerSmallSide))/(double)eltsPerSmallSide * (C-T);
        }

        if (i < eltsPerSmallSide) {
          vertices.row(3*eltsPerBigSide + i) = C+ ((double)i)/(double)eltsPerSmallSide * (L-C);
        }
        else {
