#ifndef CAPACITOR_MESHmysolution
#define CAPACITOR_MESHmysolution
...
BoundaryMesh createCapacitorMesh(int k) {
  ...
  for (int i = 0; i < numVertices / 4; ++i) {
    ...
    // Right edge
    vertices.row(numVertices / 4 + i) =
        SE + 4. * ((double)i) / (double)numVertices * (NE - SE);
    // Top edge
    vertices.row(2 * numVertices / 4 + i) =
        NE + 4. * ((double)i) / (double)numVertices * (NW - NE);
    // Left edge
    vertices.row(3 * numVertices / 4 + i) =
        NW + 4. * ((double)i) / (double)numVertices * (SW - NW);
  }
